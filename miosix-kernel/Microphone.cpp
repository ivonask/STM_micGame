...
#define SAMPLES_AT_TIME 4096
#define FFT_SIZE SAMPLES_AT_TIME / 2
...
void Microphone::mainLoop(){
	...
	arm_cmplx_mag_f32(decimatedReadyBuffer,fftBuf,FFT_SIZE); //process the data thorugh the complex magnitude module for calculating the mag of each bin
	arm_max_f32 (fftBuf,FFT_SIZE, &maxValue,&maxIndex);
...
bool Microphone::processPDM(const unsigned short *pdmbuffer, int size) {
    ...
    for (int i=0; i < length; i++){    
        ...
        if(PCMindex - (decimatedIndex * DECIMATION_FACTOR) == 0){
            decimatedProcessingBuffer[decimatedIndex] = (float)s;
        }
        PCMindex++;
    }
